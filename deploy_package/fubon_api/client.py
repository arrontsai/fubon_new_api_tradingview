...
class FubonClient:
    ...
    def __init__(self, cert_path: str = None, cert_password: str = None, personal_id: str = "", password: str = ""):
        ...
        """
        # 優先使用傳入的參數，其次使用環境變數
        self.cert_path = cert_path or os.environ.get("FUBON_CERT_PATH", settings.CERT_PATH)
        self.cert_password = cert_password or os.environ.get("FUBON_CERT_PASSWORD", settings.CERT_PASSWORD)
        self.personal_id = personal_id or os.environ.get("FUBON_PERSONAL_ID", settings.PERSONAL_ID)
        self.password = password or os.environ.get("FUBON_PASSWORD", settings.PASSWORD)
    ...
    def _get_cert_from_secrets_manager(self, secret_name):
        ...
        try:
            # 移除前綴標記符
            if secret_name.startswith('secrets:'):
            ...
            if not self.use_mock and _fubon_neo is not None:
                try:
                    ...
                    try:
                        ...
                        # 從Secrets Manager獲取憑證
                        if cert_path.startswith('secrets:'):
                            cert_path = self._get_cert_from_secrets_manager(cert_path)
                            if not cert_path:
                                raise Exception("無法從Secrets Manager獲取憑證")
                        
                        # 從Parameter Store獲取憑證
                        elif cert_path.startswith('/'):
                            cert_path = self._get_cert_from_parameter_store(cert_path)
                            if not cert_path:
                                raise Exception("無法從Parameter Store獲取憑證")
